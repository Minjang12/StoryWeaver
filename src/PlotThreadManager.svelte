<script>
    ...
    $: activeTaggingPlotThreadId = $uiState.activeTaggingPlotThreadId;
    $: activePlotThread = activeTaggingPlotThreadId ? plotThreads.find(p => p.id === activeTaggingPlotThreadId) : null;
    ...
    function toggleTagMode(threadId) {
            ...
            <div class="thread-list">
                {#if plotThreads.length > 0}
                    {#each plotThreads as thread (thread.id)}
                        <div class="thread-item" class:active={activeTaggingPlotThreadId === thread.id} animate:flip={{duration: 300}}>
                            ...
                            {:else}
                                ...
                                <button class="icon-btn" on:click|stopPropagation={() => toggleTagMode(thread.id)} title="태그 모드 전환" class:active={activeTaggingPlotThreadId === thread.id}>
...
<style>
    .plot-thread-panel {
        position: absolute;
        top: 16px;
        left: 16px; /* MIMINYAN: Changed from right to left */
        z-index: 10;
        width: 320px;
    ...
    .scene-count { font-size: 0.8em; color: var(--text-color-muted); margin-left: auto; padding-right: 8px;}
    .name-input {
    ...
    .color-input::-webkit-color-swatch { border-radius: 50%; border: 1px solid var(--border-color); }
    .color-input::-moz-color-swatch { border-radius: 50%; border: 1px solid var(--border-color); }
